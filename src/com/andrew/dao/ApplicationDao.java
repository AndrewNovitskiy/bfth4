...
public class ApplicationDao {
    ...
    private static final String SQL_GET_ALL_APPLICATIONS = "SELECT application.id_application, applicant.name, applicant.surname, vacancy.position, application_status.value, application.stat\n" +
    ...
    private static final String SQL_GET_APPLICATION_BY_ID = "SELECT application.id_application, applicant.id_applicant, vacancy.id_vacancy, applicant.name, applicant.surname, applicant.telephone, applicant.email, vacancy.position, application_status.value, application.stat\n" +
            "            FROM application LEFT JOIN applicant ON application.id_applicant = applicant.id_applicant\n" +
            "             LEFT JOIN vacancy ON application.id_vacancy = vacancy.id_vacancy\n" +
            "\t\t\t LEFT JOIN application_status ON application.id_status = application_status.id_status\n" +
            "             WHERE application.id_application = ?;";

    private static final String SQL_GET_APPLICATIONS_OF_USER = "SELECT application.id_application, applicant.name, applicant.surname, vacancy.position, application_status.value, application.stat\n" +
            "\tFROM application LEFT JOIN applicant ON application.id_applicant = applicant.id_applicant\n" +
            "\t\t\t\t\t LEFT JOIN vacancy ON application.id_vacancy = vacancy.id_vacancy\n" +
            "                     LEFT JOIN application_status ON application.id_status = application_status.id_status " +
            "WHERE application.id_applicant = ?;";

    private static final String SQL_GET_APPLICATIONS_OF_VACANCY = "SELECT application.id_application, applicant.name, applicant.surname, vacancy.position, application_status.value, application.stat\n" +
            "\tFROM application LEFT JOIN applicant ON application.id_applicant = applicant.id_applicant\n" +
            "\t\t\t\t\t LEFT JOIN vacancy ON application.id_vacancy = vacancy.id_vacancy\n" +
            "                     LEFT JOIN application_status ON application.id_status = application_status.id_status " +
    ...
    public ArrayList<Application> takeAllApplications(){
        try {
            ...
            while (rs.next()) {
                ...
                String statusEn = rs.getString("stat");

                application = new Application(applicationId, name, surname, position, status, statusEn);
    ...
    public Application takeApplicationById(Integer applicationId) {
        try {
            ...
            if (rs.next()) {
                ...
                application = new Application(applicationId, name, surname, telephone, email, position, status, statusEn, applicantId, vacancyId);
            }
            return application;
        } catch (SQLException e) {
            log.info("SQLException");
    ...
    public ArrayList<Application> takeApplicationsOfUser(int userId) {
        try {
            ...
            while (rs.next()) {
                ...
                application = new Application(applicationId, position, status, statusEn);
                applications.add(application);
            }
            return applications;
        } catch (SQLException e) {
